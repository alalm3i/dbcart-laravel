...
namespace NabeelAlalmai\DBcart\Models;
...
class Cart extends Model
{
    ...
    public static function init($instance_name, $save_on_demand)
    {
        ...
        $carts = $request->offsetGet("cart_instances");

        //if user logged in
        if ($user_id) {
            $user_cart = static::active()->user($user_id)->where(['name' => $instance_name])->first();
